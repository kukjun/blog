...
description: "용량 상한이 사실상 없는 파일시스템으로 옮기고 나니 정리 작업이 돌지 않았습니다. 보고된 총량에 비해 사용량이 너무 작아 조기 반환에 걸린 거였어요. 앞선 이관 검증은 API 호환성만 봤고, 빠진 건 용량 제한에 기대던 정리 정책이었습니다."
pubDate: 2026-07-31
...
당시 사용량으로 계산한 비율은 임계값에 닿지 않아 `return`에서 빠져나옵니다. 관측한
기간에 정리 로직이 실행되지 않은 이유였어요. 워치독은 스케줄대로 잘 돌고 있었고,
매번 "여유 있음"으로 판정하고 돌아갔습니다. 에러도 경고도 없이요.
...
flowchart LR
  ...
  A --> C[물리적 상한<br/>쓰기 실패]
  D[공유 파일시스템] --> E[정리 작업<br/>사용률이 임계값에 닿지 않음]
...
합치고, 사용량을 총 용량으로 나눈 비율과 사용량을 설정한 바이트 예산으로 나눈 비율 중
더 높은 쪽을 쓰도록 했습니다. 같은 단위의 비율로 비교하는 거예요. 볼륨의 용량이나
설정한 예산 중 먼저 가까워지는 제한에 반응하므로, 코드는 매체 종류를 몰라도 됩니다.
...
예산을 일시적으로 낮춰서 발화하는지 보고, 원복하고, 그 과정을 기록으로 남기도록
완료 조건을 잡았습니다. 이것은 검증 방법에 대한 결정이에요. 실제 발화와 원복 결과는
이 글에 제시하지 않았으므로, 여기서는 시험의 완료 여부까지 판단할 수 없습니다.
...
이번 이관에서는 파일 연산 호환성뿐 아니라, 용량을 읽는 코드와 그 값을 소비하는 정책을
함께 확인했어야 했습니다. 총 용량이 유효한 제한인지, 아니면 별도의 예산이 필요한지까지
검증 범위에 넣어야 한다는 게 남았어요.
