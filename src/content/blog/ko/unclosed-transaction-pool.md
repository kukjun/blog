...
아래 코드는 트랜잭션 경계만 보여주는 축약 예시입니다. 커넥션 생성과 업무 함수는 생략했어요.
...
당시 경로에서는 열린 트랜잭션이 다음 사용자에게 남았습니다. 모든 풀에서 같은 일이
생기는 것은 아니에요. 반환 시 rollback이나 reset을 수행하는지는 드라이버와 풀 구현에
따라 다릅니다. 이 글에는 당시 정확한 패치 버전이 없어 현재 모든 TypeORM 구성의
동작이라고 일반화하지는 않겠습니다.

이게 왜 옛날 값을 보여주는지 궁금하실 텐데요, MySQL과 MariaDB의 기본 격리 수준인
REPEATABLE READ의 일관 읽기로 설명할 수 있습니다. 일반적인 비잠금 SELECT는
트랜잭션의 첫 일관 읽기에서 정한 스냅샷을 재사용해요. 잠금 읽기와 UPDATE까지 모두
같은 옛 스냅샷만 보는 것은 아닙니다. 그러니 트랜잭션이 열린 채로 멈춰
...
async goodCode() {
  ...
  try {
    ...
    await connection.release();          // 종료 처리 후 반환, 종료 자체의 실패는 별도 처리
  }
}
```
...
감싸면 정상 반환과 예외 시 종료를 한곳에서 관리할 수 있어요. 래퍼 안의 모든 쿼리가
같은 트랜잭션 커넥션을 쓰는지, 커밋이나 롤백 자체가 실패할 때 손상된 커넥션을 어떻게
처리하는지도 확인해야 합니다. 중간 return을 금지하는 것보다 그 경로를 포함한 종료
계약을 검증하는 편이 오래 갑니다.
