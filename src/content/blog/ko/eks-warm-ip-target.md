...
sequenceDiagram
  ...
  Sched->>Node: 정시 버스트, 워커 파드 N개 생성
  Node->>EC2: 사용중 + N 맞추려고 IP 추가 할당 요청
  EC2-->>Node: IP 할당 (수 초 소요)
  Note over Node: 태스크 완료, 파드 N개 종료
...
거였어요. 크기 문제가 아니라 유지 문제였습니다. 버퍼를 키운다고 해결될 리가 없었죠.

절대 기준은 따로 있었어요. `MINIMUM_IP_TARGET`은 "사용량과 무관하게 최소 이만큼은
항상 붙여둬라"는 하한입니다. 이 값을 버스트 크기만큼 올리면, 파드가 빠져도 IP를
반납하지 않고, 다음 버스트는 EC2 API를 부르지 않고 바로 뜹니다.
...
| 단계 | 바꾼 것 | 결과 |
|---|---|---|
| 1 | ENI 단위 예약을 IP 단위로 | 노드당 32.8개에서 16개, 서브넷 여유 150에서 458 |
| 2 | 상대값 여분 상향 | 잔여 실패 1건 남음 |
| 3 | 절대 하한 설정 | 6시간 6사이클 동안 버스트 실패 8건에서 0건 |
...
하나는 상대 기준과 절대 기준이 다른 도구라는 거예요. 짧은 태스크가 빠르게 들고 나는
워크로드에 상대 기준 버퍼는 구조적으로 안 맞습니다. 버퍼가 작아서가 아니라, 버퍼가
사용량을 따라 움직이기 때문이에요.

둘은 스냅샷 한 장으로 낸 판단이 시계열을 보면 정반대일 수 있다는 거예요. "IP가 400개
남았는데 왜 못 받지"는 스냅샷의 질문이었고, 답은 시간축에 있었습니다.

셋은 문서에만 적힌 문제는 아무 일도 일으키지 않는다는 거예요. 석 달 전에 누군가 이미
알았지만 티켓이 없어서 93일을 갔습니다. 이번에 같은 상태의 항목 네 개를 더 찾아서 전부
티켓으로 만들어뒀어요.
