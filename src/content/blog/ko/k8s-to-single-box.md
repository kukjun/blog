...
그 고객 환경은 인터넷이 안 닿아요. 인바운드는 물론이고 아웃바운드도 없습니다. 서버 한
대를 주고 그 위에서 알아서 돌아가게 해달라는 거였어요.
...
애플리케이션 번들, 데이터베이스, 파일 저장소, 모델 서빙 넷이었습니다. 서버 한 대에
충분히 들어가는 크기였어요.
...
그래서 이미지를 하나 굽기로 했습니다. 애플리케이션과 데이터베이스, 서빙까지 다 들어간
머신 이미지를 만들어서 고객이 부팅만 하면 되게요. 여기서부터가 진짜였어요.
