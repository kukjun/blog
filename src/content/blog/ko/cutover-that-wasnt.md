...
새 시스템을 돌려놓고 결과만 비교하는 단계요. 마이그레이션에서 반드시 거쳐야 하는
정상적인 중간 상태고, 이 팀은 그걸 제대로 하고 있었습니다.
...
그리고 재시도를 붙일 때 타임아웃도 같이 줄여야 했어요. 실행 타임아웃이 6시간인데 재시도
3회를 붙이면 완전히 실패로 판정되기까지 18시간이 걸립니다. 알림이 반나절 넘게 늦어지는
거예요. 재시도는 복원력을 올리지만 **탐지를 늦춥니다.** 둘을 같이 조정하지 않으면 한쪽을
고치면서 다른 쪽을 망가뜨려요.
...
그리고 아무도 거짓말을 안 했다는 점도 남았어요. 그 팀은 자기가 한 일을 정확히 했고,
경로 분리는 좋은 설계였고, 소비자 코드에는 현재 상태가 주석으로 적혀 있었습니다. 각자의
자리에서는 다 맞았어요. 다만 그 조각들이 모여서 무슨 상태를 만드는지를 아무도 한 번에
안 보고 있었을 뿐입니다.

그래서 요즘은 남의 완료 보고를 일정의 근거로 쓸 때 한 번 더 물어봐요. 악의를 의심하는
게 아니라, "완료"라는 단어가 나와 그 사람에게 같은 뜻인지 확인하는 겁니다. 대개는
다르더라고요.
...
- [Airflow DAG parameters: retries and execution_timeout](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/dags.html) (Apache Airflow): 재시도와 타임아웃이 곱해져 탐지 시간이 되는 구조
