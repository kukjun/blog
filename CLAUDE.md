...
현재 재작성 원고는 `src/content/blog/{ko,en}/`의 `langgraph-pregel`, `unclosed-transaction-pool`, `redis-distributed-lock`, `prompt-cache-doubled-cost`, `session-aware-routing`, `efs-statfs-32bit` 여섯 주제다. 한국어 검토를 마친 글마다 영어판을 맞춘다. 다른 글의 과거 공개 여부를 품질 통과 근거로 삼지 않는다.

사용자가 여러 편 작성을 요청하면 한 편의 시범 완료와 전체 완료를 구별한다. 각 원고의 작성, 검증, 로컬 노출, 사용자 피드백 상태를 따로 기록하고 시범 단계의 완료 결과를 전체 작업 완료로 보고하지 않는다.

### 발행 조건
- 도입에서 왜 이 문제를 풀게 됐는지, 원하는 동작과 실제 동작이 어떻게 달랐는지 먼저 밝힌다. 사용자가 계기를 기억하지 못하면 기존 원문, 작업 기록, Git 이력에서 먼저 찾는다. AI 요약만으로 원래 동기를 확정하지 않는다.
- 원문에 명시된 동기, 같은 시기의 개발 맥락, 새 실험의 목적을 구별한다. 시기가 가까운 커밋만으로 특정 장애나 집필 계기와의 인과를 단정하지 않는다. 찾지 못한 동기를 새 실험 목적으로 대신하거나 과거 경험을 만들어 채우지 않는다.
